...
const SignUpForm = ({setOpenSignup, setOpenLogin, setShowModal}) => {
  ...
  const onSignUp = async (e) => {
    ...
    //   }
    // }
    const data = await dispatch(signUp(email, username, firstName, lastName, password, profileImg));
    if (data){
      setErrors(data)
    } else{
      setOpenSignup(false)
  ...
  return (
    <form onSubmit={onSignUp} className='signin_container'>
      ...
      <div>
        <label htmlFor='profileImg' className='signin_label' >Profile Image</label>
        <input
          type='profileImg'
          name='profileImg'
